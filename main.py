...
def main():
    ...
    with tf.device("/cpu:0"):
        ...
        with h5py.File('pretrained_models/nanoflownet/model-best.h5','r+') as f:
            print((f.attrs['model_config']))
            def update_dict(d, replace=False):
                if isinstance(d, list):
                    for v in d:
                        update_dict(v, replace)
                elif isinstance(d, dict):
                    if 'class_name' in d and d['class_name'] == 'SeparableConv2D':
                        replace = True
                    for k, v in d.items():
                        update_dict(v, replace)
                    if replace:
                        if 'kernel_initializer' in d:
                            d['depthwise_initializer'] = v
                            d['pointwise_initializer'] = v
                            del d['kernel_initializer']
                        if 'kernel_regularizer' in d:
                            d['depthwise_regularizer'] = v
                            d['pointwise_regularizer'] = v
                            del d['kernel_regularizer']
                        if 'kernel_constraint' in d:
                            d['depthwise_constraint'] = v
                            d['pointwise_constraint'] = v
                            del d['kernel_constraint']
                        if 'groups' in d:
                            del d['groups']
            ...
            print((f.attrs['model_config']))
        model = tf.keras.models.load_model('./pretrained_models/nanoflownet/model-best.h5')


        i1 = cv2.imread('2.png', cv2.IMREAD_GRAYSCALE)
        i2 = cv2.imread('1.png', cv2.IMREAD_GRAYSCALE)
        i1 = (i1 - 128) / 128
        i2 = (i2 - 128) / 128
        imgs = np.expand_dims(np.stack([i1, i2], axis=-1), 0)
        out = model.predict(imgs, verbose=1)
        np.save('out.npy', out[0][0])
